...
public class BudgetServiceImpl implements BudgetService {
    ...
    @Override
    public void tambahPengeluaran(String kategori, double jumlah, String keterangan) {
    // Gunakan class turunan dari abstract untuk OOP
    com.haidar.budgetplanner.entity.ExpenseItem item = new com.haidar.budgetplanner.entity.ExpenseItem(keterangan, jumlah);
    item.setJenis("pengeluaran");
    item.notifyUser("Pengeluaran ditambahkan: " + item.getDescription());

    // Simpan ke entitas database
    BudgetItem entity = new BudgetItem();
    entity.setKategori(kategori);
    entity.setJumlah(item.getAmount());
    entity.setKeterangan(item.getDescription());
    entity.setJenis(item.getJenis());
